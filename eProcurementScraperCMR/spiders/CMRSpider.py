...
class CMRSpider(BaseSpider):
    ...
    start_urls = [
        'https://tenders.procurement.gov.ge/engine/ssp/ssp_controller.php?action=ssp_list&search=start&ssp_page=1&_=%d' % int(
            time.time() * 1000)]
    ...
    def _process_tender(self, response):
        ...
        try:
            ...
            iProcurement['pValueDate'], iProcurement['pValueContract'] = \
            self.regex['pValueDateAndContract'].findall(allAmounts)[0]
    ...
    def _compile_regex(self):
        ...
        # Procuring Entities
        self.regex['pProcuringEntities'] = re.compile(
            ur'Procuring\s+entities.*?\<td\>(.*?)\s*(\#\d+)*\s*\((\d+)\)\<br\>.*?\<strong\>(.*?)\<', re.UNICODE)

        # Supplier
        self.regex['pSupplier'] = re.compile(
            ur'Supplier.*?\<td\>(.*)\s+\(\s*(.*?)\s*\)\<br\>.*?\<strong\>(.*?)\</strong\>', re.UNICODE)

        # Amounts
        self.regex['allAmounts'] = re.compile(ur'Amounts.*?\<table.*?\>(.*?)\</table\>', re.UNICODE)
        self.regex['pValueDateAndContract'] = re.compile(
            ur'contract\s+value.*?\>(\d{2}\.\d{2}\.\d{4})\<.*?(\d+\.*\d+\s*\w+)\<', re.UNICODE)

        self.regex['paidAmounts'] = re.compile(
            ur'actually\s+paid\s+amount.*?\>(\d{2}\.\d{2}\.\d{4})\<.*?(\d+\.*\d+\s*\w+)\<', re.UNICODE)

        # Financing source
        self.regex['pFinancingSource'] = re.compile(ur'Financing.*?\<td\>(.*?)\s*\<br\>.*?\<strong\>(.*?)\<',
                                                    re.UNICODE)

        # Procurement Base
        self.regex['pProcurementBase'] = re.compile(ur'Procurement\s+Base.*?\<td\>[&quot;]?(.*?)[&quot;]?\s*\<',
                                                    re.UNICODE)

        # Document
        self.regex['pDocument'] = re.compile(
            ur'Document.*?\<td.*?\>(.*?)\s*\<br.*?\>(#\s*.*?)\<br.*?(\d{2}\.\d{2}\.\d{4}).*?(\d{2}\.\d{2}\.\d{4}).*?(\d{2}\.\d{2}\.\d{4})\<',
            re.UNICODE)
        ...
        # CPV Codes( detailed)
        self.regex['allCodesDetailed'] = re.compile(ur'CPV\s+Codes\s+\(detailed\)\<\/div\>(.*?\<\/div\>)&nbsp;',
                                                    re.UNICODE)
